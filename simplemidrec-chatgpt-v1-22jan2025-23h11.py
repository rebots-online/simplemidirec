...
def record_midi():
    ...
    try:
        with mido.open_input(port_name) as port:
            ...
            while True:
                ...
                # Calculate relative time since last message
                current_time = time.time()
                delta_time = current_time - start_time

                # Debugging: Log delta time
                print(f"Delta time (seconds): {delta_time}")

                # Convert to MIDI ticks
                ticks_per_second = 240 / (60 / 120)  # 240 ticks per beat at 120 BPM
                msg.time = int(delta_time * ticks_per_second)

                # Debugging: Log ticks
                print(f"Delta time (ticks): {msg.time}, Message: {msg}")
    ...
    # Prevent saving an empty track
    if not track:
        print("No MIDI events recorded. Exiting without saving.")
        return
